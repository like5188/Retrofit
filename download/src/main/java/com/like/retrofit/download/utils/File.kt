...
suspend fun File?.split(count: Int): List<File>? {
    ...
    withContext(Dispatchers.IO) {
        RandomAccessFile(this@split, "rw").use { input ->
            for (i in (1..count)) {
                val outFile = File("${this@split.absolutePath}.$i")
 ...
 */
@Suppress("BlockingMethodInNonBlockingContext")
suspend fun List<File>?.merge(outFile: File?, deleteFiles: Boolean = false) {
